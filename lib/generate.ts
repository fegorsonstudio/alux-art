...
export async function startGenerationWorker(
  ...
  const newsEntry = newsFrameRef
    ? { url: newsFrameRef.url, label: "NEWS FRAME plate — a BLANK LAYOUT GUIDE for the TV news-broadcast graphics (caption bar, coloured lower-third banner, ticker strip, corner branding block). Copy ONLY the position, shape, proportions, colours and font styling of those bars. Its existing words, logos, website and handle are placeholders from an unrelated broadcast — do NOT copy any lettering from this plate; re-letter every bar with the exact text given in the prompt. The subject is the on-camera eyewitness inside the frame (no studio backdrop for this image)" }
    : null;
