...
type SlotRow = {
  ...
  slot: number;
  status: string;
  /** Which engine this row belongs to. Null means the shoot-wide default. */
...
export async function startGenerationWorker(
  ...
  const rawRefs = await sql`SELECT purpose, tag, custom_name, note, name, storage_bucket, storage_path FROM shoot_references WHERE shoot_id = ${shootId} ORDER BY created_at ASC, id ASC` as unknown as ShootRefRow[];
  // configured_model is how a slot says which engine it belongs to. Per-image
  // shoots book two rows per photo — one per model — so the row, not a global
  // setting, decides what generates it.
  const shootImages = await sql`SELECT id, slot, status, configured_model FROM shoot_images WHERE shoot_id = ${shootId}` as unknown as SlotRow[];
  ...
  const newsEntry = newsFrameRef
    ? { url: newsFrameRef.url, label: "NEWS FRAME plate — TAKE TWO THINGS FROM IT. (1) THE SETTING: recreate this exact location, environment and backdrop behind the subject, same time of day and surroundings — do not invent a different place. (2) THE OUTFIT: the on-camera person's clothing is the outfit for this image — same jacket/hoodie in the same colour and fabric, and any helmet, cap or hood reproduced faithfully and worn the same way. (3) THE GRAPHICS LAYOUT: the position, shape, proportions, colours and font styling of its on-screen bars (caption bar, coloured lower-third banner, ticker strip, corner branding block). DO NOT take its wording — the existing words, logos, website and handle are placeholders from an unrelated broadcast; re-letter every bar with the exact text given in the prompt. The subject is the on-camera eyewitness inside that frame (no studio backdrop for this image)" }
    : null;
