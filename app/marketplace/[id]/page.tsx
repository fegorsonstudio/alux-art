...
export default function TemplatePage() {
  ...
  const purchase = async () => {
    ...
    const res = await fetch(`/api/marketplace/${id}/purchase`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ couponCode: couponResult?.valid ? couponCode : undefined }),
    });
    if (res.status === 401) {
      window.location.href = `/login?redirect=/marketplace/${id}`;
      return;
        ...
        <div className={styles.infoCol}>
          ...
          <div className={styles.purchaseBox}>
            ...
            <button
              ...
              onClick={purchase}
              disabled={buying}
            >{buying ? "Redirecting to payment..." : "Book This Look"}</button>

            <p className={styles.buyNote}>
              Your identity photos + these references will be pre-loaded into your shoot. You can edit everything before generating.
            </p>
          </div>
        </div>
      </div>
